...
function generateSMSResponse(text: string, ticketNumber: string): string {
  ...
  if (lower.includes("password") || lower.includes("reset") || lower.includes("login")) {
    return `Hi! To reset your password, visit: ${process.env.NEXT_PUBLIC_APP_URL || "https://finsupport.vercel.app"}/forgot-password. Enter your email and follow the instructions. Ticket: ${ticketNumber}`;
  }
